...
def make_polynomial_feature_fun(order):
    ...
    return f
order = 1
print(f"order:{order} length: {len(make_polynomial_feature_fun(order)(np.array([[1],[2]])))}")
order = 10
print(f"order:{order} length: {len(make_polynomial_feature_fun(order)(np.array([[1],[2]])))}")
order = 20
print(f"order:{order} length: {len(make_polynomial_feature_fun(order)(np.array([[1],[2]])))}")
...
out = test_with_features(super_simple_separable_through_origin,order)
#print(out)

order = 4

#out = test_with_features(super_simple_separable,order)
#out = test_with_features(xor,order)
out = test_with_features(xor_more,order)


#out=test_with_features(xor,order,False,False)
print(out)
